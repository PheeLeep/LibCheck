    ...
    public partial class TransactStatistics : UserControl {
        ...
        internal void SelectData(DateTime from, DateTime to) {
            ...
            while (dt.Date <= to.Date) {
                ...
                if (BorrowCBox.Checked)
                    borrowRates.Add(records.Count(s => s.Category == Records.RecordStatus.BookBorrowed &&
                                                       s.DateOccurred.Date == dt.Date));
                if (ReturnCBox.Checked)
                    returnRates.Add(records.Count(s => s.Category == Records.RecordStatus.BookReturned &&
                                                       s.DateOccurred.Date == dt.Date));
